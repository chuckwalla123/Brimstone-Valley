...
import { makeBanDecision, makeMovementDecision, makePickDecision } from '../src/ai/easyAI.js';
import { executeRound } from '../src/battleEngine.js';
import { HEROES } from '../src/heroes.js';
import { makeEmptyMain, makeReserve, processMove } from '../shared/gameLogic.js';

const DEFAULT_GAMES = 1000;
const DEFAULT_MAX_ROUNDS = 60;
const DEFAULT_PROGRESS_EVERY = 100;
const DEFAULT_OUTPUT_PATH = 'scripts/easyai-selfplay-results.json';
...
function sampleHeroes(source, count) {
  const pool = Array.isArray(source) ? [...source] : [];
  for (let index = pool.length - 1; index > 0; index -= 1) {
    const swapIndex = Math.floor(Math.random() * (index + 1));
    [pool[index], pool[swapIndex]] = [pool[swapIndex], pool[index]];
  }
  return pool.slice(0, Math.max(0, Math.min(count, pool.length))).map((hero) => clone(hero));
}
...
function createInitialState() {
  return {
    ...
    bans: [],
    availableHeroes: sampleHeroes(getDraftableHeroes(), CLASSIC_DRAFT_POOL_SIZE),
...
function buildDraftQueue() {
  const actionQueue = [];
  for (const player of DRAFT_PICK_ORDER) {
    const opponent = player === 'player1' ? 'player2' : 'player1';
    actionQueue.push({ type: 'ban', player: opponent });
    actionQueue.push({ type: 'pick', player });
  }
  return actionQueue;
}

function getBoardStateForPlayer(state, player) {
  if (player === 'player1') {
    return {
      p2Main: state.p1Main,
      p2Reserve: state.p1Reserve,
      p1Main: state.p2Main,
      p1Reserve: state.p2Reserve,
    };
  }
...
function fallbackPick(availableHeroes, state, player) {
  const mainKey = player === 'player1' ? 'p1Main' : 'p2Main';
  const reserveKey = player === 'player1' ? 'p1Reserve' : 'p2Reserve';
  const main = state[mainKey] || [];
  const reserve = state[reserveKey] || [];
  const mainIndex = main.findIndex((tile) => !tile?.hero);
  if (mainIndex !== -1) {
    return { hero: availableHeroes[0], slotIndex: mainIndex, slotType: 'main' };
  }
  const reserveIndex = reserve.findIndex((tile) => !tile?.hero);
  if (reserveIndex !== -1) {
    return { hero: availableHeroes[0], slotIndex: reserveIndex, slotType: 'reserve' };
  }
  return null;
}

function withMutedEasyAIDiagnostics(action) {
  const originalError = console.error;
  console.error = (...args) => {
    const [firstArg] = args;
    if (typeof firstArg === 'string' && firstArg.startsWith('[EasyAI Draft] Invalid points')) {
      return;
    }
    originalError(...args);
  };
...
async function runDraft(state) {
  let nextState = state;
  for (const action of buildDraftQueue()) {
    if (action.type === 'ban') {
      const bannedHero = makeBanDecision(nextState.availableHeroes);
      if (!bannedHero) throw new Error(`No valid ban found for ${action.player}`);
      nextState = await processMove(nextState, { type: 'banHero', hero: clone(bannedHero) });
      continue;
    }

    const boardState = getBoardStateForPlayer(nextState, action.player);
    const aiDecision = withMutedEasyAIDiagnostics(() => makePickDecision(nextState.availableHeroes, boardState));
    const fallback = fallbackPick(nextState.availableHeroes, nextState, action.player);
    const decision = aiDecision || fallback;
    if (!decision?.hero || typeof decision.slotIndex !== 'number' || !decision.slotType) {
      throw new Error(`No valid pick found for ${action.player}`);
    }

    nextState = await processMove(nextState, {
      type: 'draftHero',
      hero: clone(decision.hero),
      player: action.player,
      tileIndex: decision.slotIndex,
      tileType: decision.slotType,
    });
  }

  return processMove(nextState, { type: 'startBattle' });
}
...
function getBoardRef(state, parsedToken) {
  ...
  if (parsedToken.side === 'p1') {
    return parsedToken.reserve ? state.p1Reserve : state.p1Main;
  }
  if (parsedToken.side === 'p2') {
    return parsedToken.reserve ? state.p2Reserve : state.p2Main;
  }
  return null;
}
...
function applyMovementDecision(state, player, decision) {
  ...
  if (!source || !destination || source.side !== moverSide || destination.side !== moverSide) {
    return state;
  }
...
async function runSingleGame(maxRounds) {
  let state = await runDraft(createInitialState());
  const draftedRosters = collectTeamHeroIds(state);
  ...
  if (!winner) {
    winner = determineWinner(state) || 'draw';
  }
...
async function main() {
  ...
  for (let gameIndex = 0; gameIndex < options.games; gameIndex += 1) {
    try {
      ...
      updateStatsForRoster(
        statsMap,
        result.draftedRosters.player1,
        result.winner === 'player1' ? 'win' : result.winner === 'player2' ? 'loss' : 'draw',
        heroLookup
      );
      updateStatsForRoster(
        statsMap,
        result.draftedRosters.player2,
        result.winner === 'player2' ? 'win' : result.winner === 'player1' ? 'loss' : 'draw',
        heroLookup
      );
    } catch (error) {
      failedGames += 1;
      console.error(`[easyai-selfplay] Game ${gameIndex + 1} failed:`, error?.stack || error?.message || error);
    }

    if (options.progressEvery > 0 && (gameIndex + 1) % options.progressEvery === 0) {
      console.log(`[easyai-selfplay] Completed ${gameIndex + 1}/${options.games} games`);
    }
  }
  ...
  console.log(`[easyai-selfplay] Wrote ${completedGames} completed games to ${outputPath}`);
  console.log(`[easyai-selfplay] Wrote CSV summary to ${csvOutputPath}`);
...
main().catch((error) => {
  console.error('[easyai-selfplay] Fatal error:', error?.stack || error?.message || error);
  process.exitCode = 1;
});