      ...
      if (tile && tile.hero && tile.hero.fixedPositional) {
        ...
        if (!tile.hero._startingRow) {
          if (isReserve) {
            ...
            try { rowIdx = indexToRow(Number(tile.index), tile.boardName.startsWith('p1') ? 'p1' : 'p2'); } catch (e) { rowIdx = null; }
      ...
      try {
        if (tile && tile.hero && tile.hero.fixedPositional && rowName === 'reserve' && tile.hero.positionalModifiers && tile.hero.positionalModifiers.reserve && typeof tile.hero.positionalModifiers.reserve.energy === 'number') {
          if (!tile._reserveBonusApplied) {
            ...
            try { console.log && console.log('[recomputeModifiers] reserve candidate', { boardName: tile.boardName, index: tile.index, heroId: tile.hero && tile.hero.id, currentEnergy: tile.currentEnergy, bonus }); } catch (e) {}
            ...
            try { console.log && console.log('[recomputeModifiers] reserve applied', { boardName: tile.boardName, index: tile.index, heroId: tile.hero && tile.hero.id, afterEnergy: tile.currentEnergy }); } catch (e) {}
          }
        }
      } catch (e) {}
    }
...
export function findTileInBoards(token, p1Board, p2Board, p1Reserve, p2Reserve) {
  ...
  return search(p1Board, 'p1Board') || search(p2Board, 'p2Board') || null;
}
...
export function applyPayloadToTarget(payload, targetRef, addLog, boards = {}, onStep = null, applyImmediately = true) {
  ...
  if (payload.action === 'damage') {
    ...
    try {
      const { p1Board = [], p2Board = [] } = boards;
      const targetBoard = targetRef.boardName && targetRef.boardName.startsWith('p1') ? p1Board : p2Board;
      const targetBoardName = targetRef.boardName && targetRef.boardName.startsWith('p1') ? 'p1' : 'p2';
      const targetIndex = targetRef.index;
      
      // Find adjacent allies with Soul Link
      const adjacentIndices = [];
      const targetRow = indexToRow(targetIndex, targetBoardName === 'p1' ? 'p1Board' : 'p2Board');
      const targetCol = ((indexToTileNumber(targetIndex, targetBoardName === 'p1' ? 'p1Board' : 'p2Board') - 1) % 3 + 3) % 3;
      ...
      for (let i = 0; i < targetBoard.length; i++) {
        ...
        const adjRow = indexToRow(i, targetBoardName === 'p1' ? 'p1Board' : 'p2Board');
        const adjCol = ((indexToTileNumber(i, targetBoardName === 'p1' ? 'p1Board' : 'p2Board') - 1) % 3 + 3) % 3;
      ...
      if (adjacentIndices.length > 0) {
        ...
        const soulLinkArmor = Number((soulLinkTile.currentArmor != null ? soulLinkTile.currentArmor : soulLinkTile.hero && soulLinkTile.hero.armor || 0) || 0);
        ...
        if (applyImmediately) {
          ...
          soulLinkRedirect = {
            boardName: targetBoardName === 'p1' ? 'p1Board' : 'p2Board',
            index: soulLinkIndex,
    ...
    if (!payload.ignoreArmor) {
      ...
      const armor = Number((tile.currentArmor != null ? tile.currentArmor : (tile.hero && tile.hero.armor) || 0) || 0);
      const reduced = Math.max(0, Math.round(v - (armor * armorMul)));
      v = reduced;
    }
    let deferredEnergyGain = 0;
    
    // Collect onTargeted reactions BEFORE damage is applied (e.g., Retribution, VengefulWards, Smolder)
      ...
      if (!shouldBypass) {
        ...
        for (let effectIdx = 0; effectIdx < allEffects.length; effectIdx++) {
          ...
          if (payload && payload.source && typeof payload.source === 'string') {
            try {
              const m = payload.source.match(/p([12])Board\[(\d+)\]/);
              if (m) {
                const b = m[1] === '1' ? 'p1' : 'p2';
                const i = Number(m[2]);
                if (ot.type === 'damage') {
                  const reactVal = Number(ot.value || 0);
                  onTargetedReactions.push({ type: 'damageAttacker', value: reactVal, effectName: effect.name, attackerBoard: b === 'p1' ? 'p1Board' : 'p2Board', attackerIndex: i, ownerBoardName: targetRef.boardName, ownerIndex: targetRef.index, effectIndex: effectIdx });
                }
                if (ot.type === 'applyEffectToAttacker') {
                  const effectName = ot.effect || ot.effectName || ot.name;
                  if (effectName) {
                    onTargetedReactions.push({ type: 'applyEffectToAttacker', effectName, attackerBoard: b === 'p1' ? 'p1Board' : 'p2Board', attackerIndex: i, ownerBoardName: targetRef.boardName, ownerIndex: targetRef.index, effectIndex: effectIdx });
                  }
                }
              }
            } catch (e) {}
          ...
          if (od.type === 'damage' && payload && payload.source && typeof payload.source === 'string') {
            try {
              ...
              if (m) {
                ...
                reactions.push({ type: 'damageAttacker', value: reactVal, effectName: effect.name, attackerBoard: b === 'p1' ? 'p1Board' : 'p2Board', attackerIndex: i, ownerBoardName: targetRef.boardName, ownerIndex: targetRef.index, effectIndex: effectIdx });
              }
            } catch (e) {}
          }
        }
...
export function applyHealthDelta(tile, delta) {
  ...
  if (!tile._passives && tile.hero && tile.hero.passives) {
    ...
    console.log('[applyHealthDelta] Initialized _passives from hero:', tile._passives);
  }
  
  const cur = Number(tile.currentHealth || 0);
  const next = cur + Number(delta || 0);
  
  console.log('[applyHealthDelta] delta:', delta, 'cur:', cur, 'next:', next, 'tile.hero:', tile.hero?.name || 'unknown');
  
  // Intercept lethal damage for one-time passives like Undying Rage
  if (next <= 0) {
    console.log('[applyHealthDelta] Lethal damage detected. Checking for Undying Rage...');
    console.log('[applyHealthDelta] tile._passives:', tile._passives);
    console.log('[applyHealthDelta] tile.hero.passives:', tile.hero?.passives);
    
    try {
      if (tile._passives && Array.isArray(tile._passives)) {
        console.log('[applyHealthDelta] _passives is array, searching...');
        const ur = tile._passives.find(p => p && (p.name === 'Undying Rage' || p.name === 'UndyingRage') && !p._used);
        console.log('[applyHealthDelta] Found Undying Rage?', ur);
        if (ur) {
          console.log('[applyHealthDelta] ✓ Undying Rage triggered! Setting HP to 1 and consuming passive.');
          ur._used = true; // consume the passive
          tile.currentHealth = 1;
          return;
        } else {
          console.log('[applyHealthDelta] ✗ No unused Undying Rage found.');
        }
      } else {
        console.log('[applyHealthDelta] ✗ tile._passives is not an array or is null/undefined');
      }
    } catch (err) {
      console.log('[applyHealthDelta] Error in Undying Rage check:', err);
    }
  }
  tile.currentHealth = capHealthForTile(tile, next);
  console.log('[applyHealthDelta] Final health:', tile.currentHealth);
}
...
export async function processMove(gameState, action, io = null, options = {}) {
  ...
  switch (action.type) {
    ...
    case 'draftHero':
      ...
      const mainKey = isP1 ? 'p1Main' : 'p2Main';
      const reserveKey = isP1 ? 'p1Reserve' : 'p2Reserve';
    ...
    case 'startRound':
      ...
      let actionSeq = 0;
      stepQueue = [];
      console.log('[processMove] startRound called with priorityPlayer:', action.priorityPlayer, 'state.priorityPlayer before:', newState.priorityPlayer);
      try {
        await executeRound(
          {
            ...
            addLog: (msg) => console.log(msg), // Server-side logging
            priorityPlayer: action.priorityPlayer || 'player1',
            roundNumber: newState.roundNumber || 1
          },
          {
            castDelayMs: 0, // No delay on server
            onStep: ({ p1Board: nb1, p2Board: nb2, p1Reserve: nr1, p2Reserve: nr2, priorityPlayer: np, lastAction }) => {
              newState.p1Main = nb1;
              newState.p2Main = nb2;
              newState.p1Reserve = nr1;
              newState.p2Reserve = nr2;
              if (np !== newState.priorityPlayer) {
                console.log('[processMove] Priority changed from', newState.priorityPlayer, 'to', np, 'during step:', lastAction?.type);
              }
        ...
        console.log('[processMove] After executeRound, priorityPlayer:', newState.priorityPlayer);
      } catch (error) {
